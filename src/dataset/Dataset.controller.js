...
const { responseServerError, responseSuccess, responseInValid, responseSuccessWithData } = require("../../helpers/ResponseRequest"); //response server
...
exports.createDataset = async (req, res) => {
    try {
        ...
        const dataDir = getDir({ dir: root + `/${DATA_FOLDER}/${req.body.dataName}` })
        Object.keys(DATA_SUBFOLDER).map((subfolder) => {
            getDir({ dir: root + `/${DATA_FOLDER}/${req.body.dataName}/${DATA_SUBFOLDER[subfolder]}` })
        })
        const data = {
            datasetId,
            dataName,
...
exports.deleteDataset = async (req, res) => {
    try {
        await dataset.findOneAndDelete({ datasetId: req.body.datasetId })
            .then(() => {
